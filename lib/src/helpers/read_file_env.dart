...
class ParserEnv {
  ...
  /// [ParserEnv] methods are pure functions.
  const ParserEnv();
...
Future<Map<String, String>> readFileEnv(String filePath) async {
  final String contents = await File(filePath).readAsString();
  const ParserEnv parser = ParserEnv();
  return parser.parse(contents.split('\n'));
}
